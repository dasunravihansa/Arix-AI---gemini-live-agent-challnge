...
import { Mic, Activity, History, StopCircle, X, Download, PenTool, Trash2, Type, ChevronRight, Eraser, WifiOff, MessageCircle } from "lucide-react";

// ─── Session ID: browser localStorage ────────────────────────────────────────
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (drawMode === "erase") { ctxRef.current.strokeStyle = "#ffffff"; ctxRef.current.lineWidth = 20; }
    else { ctxRef.current.strokeStyle = drawColor; ctxRef.current.lineWidth = 4; }
  }, [drawColor, drawMode]);
  ...
  const startDrawing = (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
    ...
    const { offsetX, offsetY } = getCoordinates(e);
    ctxRef.current.beginPath(); ctxRef.current.moveTo(offsetX, offsetY); setIsDrawing(true);
  };
  ...
  const stopDrawing = () => { ctxRef.current?.closePath(); setIsDrawing(false); };
  ...
  useEffect(() => {
    const check = () => {
      setShowExtensionPopup(document.documentElement.getAttribute("data-arix-extension-installed") !== "true");
    };
  ...
  const speak = (text: string) => {
    ...
    const doSpeak = () => {
      ...
      const v = voices.find(v => (v.name.includes("Google") || v.name.includes("Microsoft")) && v.lang.startsWith("en"))
  ...
  useEffect(() => () => { chatWsRef.current?.close(); }, []);

  // ─── Audio Playback ───────────────────────────────────────────────────────
  const playNextAudioChunk = () => {
    if (!audioContextRef.current || !audioQueueRef.current.length) {
  ...
  useEffect(() => {
    const handler = (e: MessageEvent) => {
      if (e.data?.type === "ARIX_SCREEN_CAPTURED" && wsRef.current?.readyState === WebSocket.OPEN) {
        wsRef.current.send(JSON.stringify({ type: "realtime_input", image: e.data.dataUrl }));
      }
    };
  ...
  const stopLiveSession = () => {
    setIsLive(false);
    setArixState("idle"); arixStateRef.current = "idle";
    ...
    recordingContextRef.current?.close(); recordingContextRef.current = null;
    audioContextRef.current?.close(); audioContextRef.current = null;

    audioQueueRef.current = []; isPlayingRef.current = false; whiteboardDirtyRef.current = false;
  };
  ...
  const setupAudioProcessing = (ws: WebSocket, stream: MediaStream) => {
    ...
    const micSource2 = recCtx.createMediaStreamSource(stream);
  ...
  const toggleLive = async () => {
    if (isLive) { console.log("Ending Live Voice Session..."); stopLiveSession(); return; }
    ...
    try {
      ...
      try {
        ...
        stopLiveSession(); setLiveError("🎵 Audio context failed."); setTimeout(() => setLiveError(null), 6000); return;
      }

      const sessionId = getSessionId();
      const wsBase = process.env.NEXT_PUBLIC_WS_URL || "wss://arix-backend-103963879704.us-central1.run.app";
      const wsUrl = `${wsBase.replace(/\/ws\/.*$/, "").replace(/\/$/, "")}/ws/live?session_id=${sessionId}`;
      console.log(`[WS] Connecting to: ${wsUrl} with session: ${sessionId}`);

      const ws = new WebSocket(wsUrl);
      wsRef.current = ws;
      let wasConnected = false;

      const reconnectWebSocket = async (currentSessionId: string) => {
        ...
        const saved = localStorage.getItem(`arix_history_${currentSessionId}`);
        let latestHistory = currentHistory;
        if (saved && !currentHistory.length) {
          try { latestHistory = JSON.parse(saved); } catch(e) {}
        }
        
        const wsBase2 = process.env.NEXT_PUBLIC_WS_URL || "wss://arix-backend-103963879704.us-central1.run.app";
        const baseUrl = wsBase2.replace(/\/ws\/.*$/, "").replace(/\/$/, "");
        const wsUrl2 = `${baseUrl}/ws/live?session_id=${currentSessionId}`;
        
        const ws2 = new WebSocket(wsUrl2);
        wsRef.current = ws2;
        let wasConnected2 = false;
        ...
        ws2.onopen = () => {
          ...
          if (latestHistory.length > 0) {
            const lastFew = latestHistory.slice(-5);
            const contextMsg = lastFew.map(m => 
              `${m.role === 'user' ? 'User' : 'Arix'}: ${m.text}`
            ).join('\n');
            
            console.log("📨 Sending context after reconnect...");
            ws2.send(JSON.stringify({
              type: "realtime_input",
              text: `[CONTEXT - Previous conversation before disconnect]:\n${contextMsg}\n\nContinue from where we left off.`
            }));
          }

          setupAudioProcessing(ws2, streamRef.current!);
        };
        ...
        ws2.onclose = (e2) => {
          ...
          if (e2.code !== 1000 && wasConnected2) {
            ...
            setTimeout(() => reconnectWebSocket(currentSessionId), 2000);
          } else {
            stopLiveSession();
          }
        };
      ...
      ws.onopen = () => {
        ...
        const visualCaptureLoop = setInterval(() => {
          ...
          if (showWhiteboardRef.current && canvasRef.current && whiteboardDirtyRef.current) {
            ws.send(JSON.stringify({ type: "realtime_input", image: canvasRef.current.toDataURL("image/jpeg", 0.5) }));
      ...
      ws.onclose = (event) => {
        ...
        if (!wasConnected) {
          ...
          wsRef.current = null;
          setTimeout(() => reconnectWebSocket(sessionId), 1000);
        }
      };
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#F5F5F7] font-sans relative overflow-hidden">
        ...
        {showExtensionPopup && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20, opacity: 0 }}
              ...
              <button onClick={() => setShowExtensionPopup(false)}
                className="absolute top-4 right-4 z-10 p-2 bg-black/20 hover:bg-black/40 text-white rounded-full transition-all">
                <X size={18} strokeWidth={2.5} />
              </button>
              <div className="relative h-56 w-full bg-linear-to-br from-blue-100 to-purple-100 overflow-hidden">
                <img src="/extension_promo.png" alt="Arix Extension" className="w-full h-full object-cover absolute" />
              ...
              <div className="px-8 pb-8 pt-4 text-center bg-white">
                <h2 className="text-2xl font-black text-gray-900 mb-2">Upgrade Your AI Tutor</h2>
                <p className="text-gray-500 mb-6 text-sm leading-relaxed">Install the Arix Chrome Extension to interact with your screen!</p>
                <button onClick={() => setShowExtensionPopup(false)}
                  className="w-full flex justify-center items-center gap-2 bg-linear-to-r from-[#7ba2e8] to-[#608ee1] text-white py-4 px-6 rounded-2xl font-bold shadow-lg hover:-translate-y-1 transition-all">
                  <Download size={22} /> Download Arix Extension
                </button>
                <button onClick={() => setShowExtensionPopup(false)} className="mt-4 text-sm text-gray-400 hover:text-gray-600">Maybe Later</button>
              </div>
            </motion.div>
          </motion.div>
        )}
      ...
      <motion.button initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        ...
        Chat {chatMessageCount > 0 && <span className="bg-blue-500 text-white text-xs rounded-full px-1.5 py-0.5">{chatMessageCount}</span>}
      </motion.button>

      <motion.button initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
        whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }}
        onClick={() => setShowWhiteboard(true)}
        className="absolute top-8 right-8 flex items-center gap-2 bg-linear-to-r from-blue-500 to-indigo-600 text-white font-semibold text-sm px-5 py-2.5 rounded-xl shadow-lg hover:-translate-y-1 transition-all z-10">
            ...
            <div className="bg-white w-full h-full flex flex-col">
              <div className="flex justify-between items-center py-4 px-6 bg-white border-b border-gray-100 shadow-sm">
                <div className="flex items-center gap-3">
                  <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2"><PenTool size={20} className="text-blue-500" />AI Whiteboard</h2>
                  <span className="px-3 py-1 bg-green-100 text-green-700 text-xs font-bold rounded-full animate-pulse">Visible to Arix</span>
                </div>
                <button onClick={() => { setShowWhiteboard(false); ctxRef.current = null; }}
              ...
              <div className="flex-1 relative overflow-hidden">
                <motion.div initial={false} animate={{ x: isToolbarOpen ? 0 : -80 }}
                  ...
                  <div className="w-20 bg-white rounded-r-3xl shadow-lg border border-gray-100 p-4 flex flex-col items-center gap-4 py-6">
                    {[["draw", <PenTool size={20} />], ["text", <Type size={20} />], ["erase", <Eraser size={20} />]].map(([mode, icon]) => (
                      <button key={mode as string} onClick={() => setDrawMode(mode as any)}
                        className={`p-3 flex justify-center rounded-xl transition-all ${drawMode === mode ? (mode === "erase" ? "bg-gray-800 text-white" : "bg-blue-100 text-blue-600") : "text-gray-500 hover:bg-gray-100"}`}>
                        {icon as React.ReactNode}
                      </button>
                    ))}
                    <div className="w-full h-px bg-gray-100" />
                    {["#1f2937", "#ef4444", "#3b82f6", "#10b981", "#f59e0b"].map(color => (
                      <button key={color} onClick={() => { setDrawColor(color); setDrawMode("draw"); }}
                    ...
                    <div className="w-full h-px bg-gray-100" />
                    <button onClick={clearBoard} className="p-3 text-red-500 hover:bg-red-50 rounded-xl transition-colors"><Trash2 size={20} /></button>
                  </div>
                ...
                <canvas ref={canvasRef} onClick={handleCanvasClick} onMouseDown={startDrawing} onMouseMove={draw}
                  onMouseUp={stopDrawing} onMouseLeave={stopDrawing} onTouchStart={startDrawing} onTouchMove={draw} onTouchEnd={stopDrawing}
                ...
                {textInput.visible && (
                  <form onSubmit={handleTextSubmit} className="absolute z-20" style={{ left: textInput.x, top: textInput.y - 14 }}>
                    <input type="text" autoFocus
                      className="bg-transparent text-[24px] font-bold outline-none border-b-2 px-1 min-w-50"
                      style={{ color: drawColor, borderColor: drawColor }}
                      value={textInput.value} onChange={e => setTextInput({ ...textInput, value: e.target.value })}
                      onBlur={handleTextSubmit} placeholder="Type here..." />
                  </form>
                )}
              </div>
            </div>
      ...
      <main className="text-center mb-24 z-10 w-full px-6 flex flex-col items-center justify-center min-h-[45vh]">
        <AnimatePresence mode="wait">
          {!isLive ? (
            ...
            <motion.div key="orb" initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.5, filter: "blur(10px)" }} transition={{ duration: 0.8, type: "spring" }}
          ...
          <AnimatePresence>
            {liveError && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}
                ...
                <button onClick={() => setLiveError(null)} className="p-1 rounded-lg hover:bg-red-100"><X size={14} /></button>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={toggleLive}
            ...
            {isLive ? (
              ...
              <>
                ...
                <Activity size={24} className="text-gray-300 group-hover:text-[#7ba2e8]/50 ml-2" />
              </>
            )}
          </motion.button>

          <button onClick={openChat} className="text-sm font-semibold text-blue-500 hover:text-blue-600 underline underline-offset-2 transition-colors">
            Try Text Chat (with Voice)
          </button>
        </div>
        <p className="mt-4 text-xs text-gray-400 select-none">Arix AI can make mistakes. Consider verifying important information.</p>
      </motion.div>
      ...
      <AnimatePresence>
        {showChat && (
          <motion.div initial={{ opacity: 0, y: 40, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }}
            ...
            className="fixed bottom-6 right-6 w-[95vw] sm:w-96 max-h-[85vh] sm:max-h-130 bg-white rounded-3xl shadow-2xl border border-gray-100 flex flex-col overflow-hidden z-50">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-linear-to-r from-blue-50 to-indigo-50">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-green-500 font-semibold">Memory enabled ✨</p>
                </div>
              </div>
              <button onClick={() => setShowChat(false)} className="p-1.5 rounded-full hover:bg-gray-100 text-gray-400">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="px-4 py-3 border-t border-gray-100 flex gap-2">
              <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)}
                onKeyDown={e => e.key === "Enter" && sendChatMessage()} placeholder="Ask Arix..."
